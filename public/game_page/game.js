var getParameters = function (paramName) {
    ...
    function npcAnswer (i) {
        ...
        $messageRowSendDiv1.append('<span class="message__author">선배님</span>');
        $messageRowSendDiv1.append($messageRowSendDiv2);
        $messageRowSendDiv2.append(`<span class="message__bubble">${i}<span>`);
        $('.main-chat').append($messageRowSend);
    }
    ...
    function ifAnswerIsNotCorrect(){
        npcAnswer('못 맞추셨네요ㅠㅠ');
        $('input').attr('placeholder', '작성 완료!');
        $('.fail').addClass('show-opacity');
        $('.fail > .score').html('7점 만점에 0점...');
        return;
